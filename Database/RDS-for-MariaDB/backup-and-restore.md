...
RDS for MariaDBのバックアップファイルを利用して直接MariaDBのデータベースを復元することができます。RDS for MariaDBのバックアップファイルを復元する時、[バックアップ](backup-and-restore/#_1)項目を参照してRDS for MariaDBで使用するPercona XtraBackupと同じバージョンを使用する必要があります。
...
mariabackup --decompress {MariaDBデータの保存パス}
